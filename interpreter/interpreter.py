...
from interpreter.values import SymbolTable, Context, Number, String, List
...
class Interpreter:
    ...
    def visit_ForNode(self, node, context):
        ...
        return res.success(
            Number.null if node.should_return_null else List(elements).set_context(context).set_pos(
                node.pos_start, node.pos_end))
...
class BuiltInFunction(BaseFunction):
    ...
    def execute_set(self, exec_ctx):
        ...
        if not isinstance(index, Number):
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                "Secone argument must be a number",
                exec_ctx
            ))
